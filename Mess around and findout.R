...
#Pull stats 
TBstats <- read_html("https://www.nfl.com/players/tom-brady/stats/career") %>% 
  html_nodes("td.nfl-t-stats__col-16") %>% 
  html_text(trim = T) %>% 
  str_squish()

StatCol <- read_html("https://www.nfl.com/players/tom-brady/stats/career") %>% 
  html_nodes("th.nfl-t-stats__col-16") %>% 
  html_text(trim = T) %>% 
  str_squish()
...
#Labels
xlab <- c('YEAR','TEAM','G','ATT','COMP','PCT',
          'YDS','AVG','LNG','TD','INT','1st','1st%',
          '20+','SCK','SCKY','RATE')

ylab <- c('2000','2001','2002','2003','2004','2005','2006',
